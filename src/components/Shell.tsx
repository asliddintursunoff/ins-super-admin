...
export function Shell({ children, userLabel }: { children: React.ReactNode; userLabel?: string }) {
      ...
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-4 py-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="grid h-9 w-9 place-items-center rounded-2xl bg-slate-900 text-white font-black shrink-0">IG</div>
            <div className="min-w-0">
              <div className="text-sm font-semibold leading-4">INS Grades Admin</div>
              <div className="text-xs text-slate-500 leading-4 truncate">{userLabel || "Superuser panel"}</div>
            </div>
          </div>
        ...
        {mobileOpen ? (
          <div className="md:hidden">
            <div className="fixed inset-0 z-30 bg-black/30" aria-hidden="true" onClick={() => setMobileOpen(false)} />
            ...
            <div className="fixed right-0 top-0 z-40 h-full w-[84%] max-w-sm border-l border-slate-200 bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-3 border-b border-slate-200 p-4">
                <div className="min-w-0">
                  ...
                  {userLabel ? <div className="mt-1 text-xs text-slate-500 break-all">{userLabel}</div> : null}
                </div>
                <button
                  type="button"
